#!/usr/bin/env python3
"""Post-hoc rerun only the final outer backbone answer with DeepSeek Reasoner."""
...
def build_deepseek_final_messages(row: dict[str, Any]) -> tuple[list[dict[str, str]], str, int]:
...
def process_one(row: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    messages, last_policy_output, final_from_round = build_deepseek_final_messages(row)
    ...
    if args.run_judge and final_answer:
        judge_result = judge_backbone_final_answer(
            question=str(row.get("question") or ""),
        ...
        api_stats.append({
            "stage": "deepseek_final_answer_judge",
    ...
    return {
        "record_type": "deepseek_final_round_rerun",
        ...
        "deepseek_final_output": response_text,
        "deepseek_final_answer": final_answer,
        "deepseek_final_em": final_em,
        "deepseek_final_f1": final_f1,
        "deepseek_final_answer_llm_judge_score": judge_result.get("score"),
        "deepseek_final_answer_llm_judge_is_correct": judge_result.get("is_correct"),
        "deepseek_final_answer_llm_judge_response": judge_result.get("response", ""),
        "deepseek_final_answer_llm_judge_error": judge_result.get("error"),
        "deepseek_final_from_policy_round": final_from_round,
        "deepseek_final_last_policy_output": last_policy_output if args.save_prompt_context else None,
        "deepseek_final_messages": messages if args.save_prompt_context else None,
...
def summarize(output_path: Path, summary_path: Path, original_summary_path: Optional[Path]) -> dict[str, Any]:
    ...
    ems = [float(r["deepseek_final_em"]) for r in rows if r.get("deepseek_final_em") is not None]
    f1s = [float(r["deepseek_final_f1"]) for r in rows if r.get("deepseek_final_f1") is not None]
    judges = [float(r["deepseek_final_answer_llm_judge_score"]) for r in rows if r.get("deepseek_final_answer_llm_judge_score") is not None]
    ...
    summary = {
        ...
        "deepseek_final_em_mean": statistics.fmean(ems) if ems else None,
        "deepseek_final_f1_mean": statistics.fmean(f1s) if f1s else None,
        "deepseek_final_answer_llm_judge_score_mean": statistics.fmean(judges) if judges else None,
        "deepseek_final_answer_llm_judge_scored_count": len(judges),
...
def main() -> None:
    ...
    parser.add_argument("--backbone-judge-api-url", default="https://api.deepseek.com/v1")
    parser.add_argument("--backbone-judge-model", default="deepseek-reasoner")
    parser.add_argument("--backbone-judge-api-key", default="")
    parser.add_argument("--backbone-judge-max-tokens", type=int, default=4096)
