...
OUTPUT_ROOT="${OUTPUT_ROOT:-/ai/cqn/s3/ckpt/search_subagent_qwen3_32b_1p7b_test_all}"
EXPERIMENT_NAME="${EXPERIMENT_NAME:-test_all_qwen3_32b_backbone_qwen3_1p7b_policy_${RUN_TS}}"
...
POLICY_MODEL_PATH="${POLICY_MODEL_PATH:-/ai/cqn/model/Qwen3-1.7B/}"
POLICY_MODEL="${POLICY_MODEL:-Qwen3-1.7B}"
POLICY_API_URL="${POLICY_API_URL:-http://127.0.0.1:8001/v1}"
POLICY_HOST="${POLICY_HOST:-0.0.0.0}"
POLICY_PORT="${POLICY_PORT:-8001}"
POLICY_CUDA_VISIBLE_DEVICES="${POLICY_CUDA_VISIBLE_DEVICES:-2}"
POLICY_TENSOR_PARALLEL_SIZE="${POLICY_TENSOR_PARALLEL_SIZE:-1}"
POLICY_GPU_MEMORY_UTILIZATION="${POLICY_GPU_MEMORY_UTILIZATION:-0.7}"
...
START_BACKBONE_VLLM="${START_BACKBONE_VLLM:-true}"
START_POLICY_VLLM="${START_POLICY_VLLM:-true}"
...
POLICY_VLLM_LOG="${POLICY_VLLM_LOG:-$EXP_DIR/policy_vllm_${POLICY_PORT}.log}"
...
POLICY_VLLM_PID=""
cleanup_vllm() {
  for pid_name in BACKBONE_VLLM_PID POLICY_VLLM_PID; do
    local pid="${!pid_name:-}"
...
if [ "$START_POLICY_VLLM" = "true" ]; then
  start_vllm_server \
    "$POLICY_MODEL_PATH" \
    "$POLICY_MODEL" \
    "$POLICY_HOST" \
    "$POLICY_PORT" \
    "$POLICY_CUDA_VISIBLE_DEVICES" \
    "$POLICY_TENSOR_PARALLEL_SIZE" \
    "$POLICY_GPU_MEMORY_UTILIZATION" \
    "$POLICY_API_URL" \
    "$POLICY_VLLM_LOG" \
    POLICY_VLLM_PID
else
...
cat >"$EXP_DIR/experiment_config.json" <<EOF
{
  ...
  "policy_cuda_visible_devices": "$POLICY_CUDA_VISIBLE_DEVICES",
  "policy_gpu_memory_utilization": $POLICY_GPU_MEMORY_UTILIZATION,
  "output_dir": "$OUT_DIR"
}
EOF

echo "============================================"
echo "Qwen3-32B backbone + Qwen3-1.7B policy test_all rollout"
echo "Input:              $INPUT"
...
echo "Policy vLLM log:    $POLICY_VLLM_LOG"
echo "============================================"
