...
class TaskRunner:
    ...
    def add_backbone_rollout_worker(self, config, actor_rollout_cls):
        """Optionally add a frozen backbone rollout worker group.

        Controlled by `actor_rollout_ref.rollout.custom.enable_backbone_rollout`.
        """
        from verl.trainer.ppo.ray_trainer import Role

        rollout_custom = config.actor_rollout_ref.rollout.get("custom", None) or {}
        enable_backbone_rollout = rollout_custom.get("enable_backbone_rollout", False)
        if not enable_backbone_rollout:
            return

        use_legacy_worker_impl = config.trainer.get("use_legacy_worker_impl", "auto")
        if use_legacy_worker_impl == "disable":
            raise NotImplementedError("Backbone rollout currently supports legacy workers only.")

        # Prefer a dedicated worker with optional API orchestration support on FSDP.
        if config.actor_rollout_ref.actor.strategy in {"fsdp", "fsdp2"}:
            from verl.workers.fsdp_workers import AsyncBackboneRolloutWorker

            backbone_rollout_cls = AsyncBackboneRolloutWorker
        else:
            # Fallback to the same rollout worker class for non-FSDP strategies.
            backbone_rollout_cls = actor_rollout_cls

        self.role_worker_mapping[Role.BackboneRollout] = ray.remote(backbone_rollout_cls)
        self.mapping[Role.BackboneRollout] = "global_pool"
    ...
    def run(self, config):
        ...
        self.add_backbone_rollout_worker(config, actor_rollout_cls)
        ...
        # Tokenizer assets may live in a different directory from model weights
        # (e.g., FSDP checkpoint for model.path + base model dir for tokenizer_path).
        tokenizer_src_path = config.actor_rollout_ref.model.get("tokenizer_path", config.actor_rollout_ref.model.path)
        local_tokenizer_path = copy_to_local(
            tokenizer_src_path, use_shm=config.actor_rollout_ref.model.get("use_shm", False)
        )
