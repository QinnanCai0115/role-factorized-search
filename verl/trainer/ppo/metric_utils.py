...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    def _safe_numeric_stats(arr: np.ndarray) -> tuple[float, float, float] | None:
        vals = []
        ...
        return float(vals_np.min()), float(vals_np.max()), float(vals_np.mean())

    # multi-turn conversation
    if "__num_turns__" in batch.non_tensor_batch:
        num_turns = batch.non_tensor_batch["__num_turns__"]
        stats = _safe_numeric_stats(num_turns)
        if stats is not None:
            metrics["num_turns/min"], metrics["num_turns/max"], metrics["num_turns/mean"] = stats

    if "tool_call_counts" in batch.non_tensor_batch:
        tool_call_counts = batch.non_tensor_batch["tool_call_counts"]
        stats = _safe_numeric_stats(tool_call_counts)
        if stats is not None:
            metrics["tool_call_counts/min"], metrics["tool_call_counts/max"], metrics["tool_call_counts/mean"] = stats
...
def process_validation_metrics(
    ...
    def _is_numeric_metric_value(value: Any) -> bool:
        """Only aggregate scalar numeric values into validation metrics."""
        if isinstance(value, (str, bytes, dict, list, tuple, set)):
            return False
        if isinstance(value, np.ndarray):
            return value.ndim == 0 and np.issubdtype(value.dtype, np.number)
        return np.isscalar(value) and np.issubdtype(np.asarray(value).dtype, np.number)
    ...
    for data_source, uid2var2vals in data_src2uid2var2vals.items():
        ...
        for uid, var2vals in uid2var2vals.items():
            ...
            for var_name, var_vals in var2vals.items():
                # Only scalar numeric values are valid for aggregated validation metrics.
                # Structured payloads such as log dictionaries are kept for JSONL dumping
                # but skipped here so they never reach numpy reductions.
