...
class SFTTrainer:
    ...
    def fit(self):
        ...
        for epoch in range(start_epoch, self.config.trainer.total_epochs):
            ...
            for step_in_epoch, data in enumerate(train_pbar):
                ...
                if is_logging:
                    ...
                    completed_steps = max(global_step - start_global_step, 1)
                    avg_step_time = elapsed / completed_steps
                    eta_seconds = max(self.total_training_steps - global_step, 0) * avg_step_time
                    postfix = {
                        "step": f"{global_step}/{self.total_training_steps}",
                        "elapsed": self._format_seconds(elapsed),
                        "eta": self._format_seconds(eta_seconds),
                    }
                    if step_metrics.get("loss") is not None:
                        postfix["loss"] = f"{float(step_metrics['loss']):.4f}"
                    if step_metrics.get("lr") is not None:
                        postfix["lr"] = f"{float(step_metrics['lr']):.2e}"
                    if step_metrics.get("grad_norm") is not None:
                        postfix["grad"] = f"{float(step_metrics['grad_norm']):.2f}"
                    if step_metrics.get("global_tokens") is not None:
                        postfix["tokens"] = int(step_metrics["global_tokens"])
                ...
                if is_last_step and self.val_dataloader is not None or (self.test_freq > 0 and is_valid_step):
                    # Perform validation
                    if is_logging:
                        train_pbar.write(f"Running validation at global_step={global_step}...")
                    val_losses = []
                    val_iter = tqdm(
                        self.val_dataloader,
                    ...
                    if is_logging:
                        ...
                        train_pbar.write(f"Validation step {global_step}: val/loss={metric['val/loss']:.4f}")
                    torch.distributed.barrier()

                if is_last_step or (self.save_freq > 0 and is_save_step):
                    aggressive_empty_cache(force_sync=True)
                    if is_logging:
                        train_pbar.write(f"Saving checkpoint at global_step={global_step}...")
