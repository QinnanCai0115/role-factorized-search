...
class BroadcastOperation:
    ...
    Args:
        ...
        bucket (object): The tensor to broadcast.
...
class NCCLCheckpointEngine(CheckpointEngine):
    ...
    def prepare(self) -> MasterMetadata:
        # For master process, prefer cupy to avoid memory register errors
        # when `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True`. Some
        # environments do not have a working cupy wheel, so fall back to
        # torch CUDA buffers; this is slower but keeps disaggregated sync usable.
    ...
    async def send_weights(self, weights: Generator[tuple[str, torch.Tensor], None, None]):
        ...
        for name, weight in weights:
            ...
            if cp is not None and not isinstance(send_buf, torch.Tensor):
                ...
                send_buf[offset : offset + weight.nbytes].copy_(weight_bytes, non_blocking=True)
