...
def _get_libc():
    ...
    if _LIBC is not None:
        return _LIBC
    # Prefer using the main program handle (no dlclose) when possible.
    # Fallback to libc.so.6 when malloc_trim isn't available via CDLL(None).
    try:
...
def _release_update_weight_memory() -> None:
    gc.collect()
    ...
    async def update_weights(self, global_steps: int = None):
        ...
        if self.config.rollout.checkpoint_engine.backend != "naive":
            ...
            try:
                ...
                close = getattr(per_tensor_param, "close", None)
                if callable(close):
                    close()
                del per_tensor_param
                del peft_config
                _release_update_weight_memory()
            return
