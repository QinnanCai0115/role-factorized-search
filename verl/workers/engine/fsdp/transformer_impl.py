...
class FSDPEngine(BaseEngine):
    ...
    def _build_lora_module(self, module):
        ...
        lora_dtype = self.engine_config.model_dtype
        if lora_dtype is None:
            lora_dtype = torch.float32 if not self.engine_config.forward_only else torch.bfloat16
        lora_dtype = PrecisionType.to_dtype(lora_dtype)
        for param in module.parameters():
            if param.requires_grad and param.is_floating_point() and param.dtype != lora_dtype:
