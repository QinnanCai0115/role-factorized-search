...
class SeparateRayPPOTrainer(RayPPOTrainer):
    ...
    def _ensure_io_trace_defaults(self) -> None:
        """Initialize trace fields for async trainers that do not call RayPPOTrainer.__init__."""
        if hasattr(self, "io_trace_log_path"):
            return
        rollout_custom = self.config.actor_rollout_ref.rollout.get("custom", None) or {}
        self.io_trace_log_path = str(
            rollout_custom.get("io_trace_log_path", os.getenv("VERL_IO_TRACE_LOG_PATH", "")) or ""
        )
    ...
    def _fit_append_io_trace(self, event: str, payload: dict[str, Any]) -> None:
        self._ensure_io_trace_defaults()
        self._append_io_trace(event, payload)

    def _fit_prepare_policy_batch(self, batch: DataProto) -> DataProto:
        """Match the synchronous trainer's post-rollout preparation before reward/logprob/update."""
        ...
        if self.config.algorithm.get("tool_reward_as_grpo_point", False):
            ...
            metrics["tool_reward_expansion/num_with_tool_rewards"] = expansion_summary_before["num_with_tool_rewards"]
            metrics["tool_reward_expansion/num_without_tool_rewards"] = expansion_summary_before[
                "num_without_tool_rewards"
            ]
            metrics["tool_reward_expansion/total_reward_points"] = expansion_summary_before["total_reward_points"]
            self._fit_append_io_trace(
        ...
        if not has_policy_train_points:
            return batch
    ...
    def _fit_generate(self, batch: DataProto = None) -> DataProto:
        ...
        return self._fit_prepare_policy_batch(batch)
    ...
    def _fit_compute_reward(self, batch: DataProto) -> DataProto:
        ...
        with marked_timer("reward", timing_raw, color="yellow"):
            ...
            if backbone_em is not None and backbone_f1 is not None:
                ...
                self.metrics["train-aux/backbone_final_em/mean"] = float(np.mean(backbone_em))
                self.metrics["train-aux/backbone_final_f1/mean"] = float(np.mean(backbone_f1))
                self.metrics["train-aux/backbone_final_em/max"] = float(np.max(backbone_em))
                self.metrics["train-aux/backbone_final_f1/max"] = float(np.max(backbone_f1))
