...
SEARCH_XML_REGEX = regex.compile(r"<search>\s*(.*?)\s*</search>", regex.DOTALL | regex.IGNORECASE)
SEARCH_TOOL_NAME = "search_subagent"
...
class ToolParser(ABC):
    ...
    @staticmethod
    def _preview_text(text: Any, max_chars: int = 400) -> str:
    ...
    @staticmethod
    def _normalize_tool_call_payload(payload: Any) -> str:
        text = str(payload if payload is not None else "").strip()
        fenced_match = regex.match(r"^```(?:json)?\s*(.*?)\s*```$", text, flags=regex.DOTALL | regex.IGNORECASE)
        if fenced_match:
            text = fenced_match.group(1).strip()
        return text

    def _log_tool_call_decode_error(
        self,
        *,
        parser_name: str,
        error: Exception,
        ...
        self._last_decode_error = str(error)
        ctx = parse_context or {}
        logger.error(
            "Failed to decode tool call [parser=%s request_id=%s trace_id=%s round=%s assistant_turn=%s]: %s; "
            "raw_tool_call=%s; response_preview=%s",
            parser_name,
            ctx.get("request_id", ""),
            ctx.get("trace_id", ""),
            ctx.get("orchestrator_round", None),
            ctx.get("assistant_turn", None),
            error,
            self._preview_text(raw_tool_call),
            self._preview_text(response_text, max_chars=800),
        )

    def pop_last_decode_error(self) -> Optional[str]:
        error = self._last_decode_error
...
def extract_search_xml_tool_calls(text: str) -> tuple[str, list[FunctionCall]]:
    function_calls: list[FunctionCall] = []
    for match in SEARCH_XML_REGEX.finditer(text or ""):
        query = match.group(1).strip()
        if query:
            function_calls.append(
                FunctionCall(name=SEARCH_TOOL_NAME, arguments=json.dumps({"query": query}, ensure_ascii=False))
            )
    if not function_calls:
        return text, []
    return SEARCH_XML_REGEX.sub("", text or ""), function_calls


@ToolParser.register("search_xml")
class SearchXMLToolParser(ToolParser):
    """Parse simple search tool calls in the form <search>query</search>."""
    ...
    async def extract_tool_calls(
        ...
        if self.tool_call_start_token not in text:
            return text, []
        return extract_search_xml_tool_calls(text)
...
class HermesToolParser(ToolParser):
    ...
    def __init__(self, tokenizer) -> None:
        ...
        self.xml_fallback_parser = Qwen3XMLToolParser(tokenizer)
    ...
    async def extract_tool_calls(
        ...
        for match in matches:
            normalized_match = self._normalize_tool_call_payload(match)
            try:
                function_call = json.loads(normalized_match)
                name, arguments = function_call["name"], function_call["arguments"]
                function_calls.append(FunctionCall(name=name, arguments=json.dumps(arguments, ensure_ascii=False)))
            except Exception as e:
                if normalized_match.startswith("<function="):
                    try:
                        function_calls.append(self.xml_fallback_parser._parse_xml_function_call(normalized_match, tools))
                        logger.warning(
                            "Recovered Hermes tool call via XML fallback [request_id=%s trace_id=%s round=%s]",
                            (parse_context or {}).get("request_id", ""),
                            (parse_context or {}).get("trace_id", ""),
                            (parse_context or {}).get("orchestrator_round", None),
                        )
                        continue
                    except Exception:
                        pass
                self._log_tool_call_decode_error(
                    parser_name="hermes",
                    error=e,
                    raw_tool_call=match,
                    response_text=text,
