...
class MessageQueue:
    ...
    def __init__(self, config: DictConfig, max_queue_size: int = 1000):
        ...
        self.total_produced_train_points = 0
        self.total_consumed_train_points = 0
        self.dropped_samples = 0
        self.dropped_train_points = 0

        print(f"[MessageQueue] initialized with max_queue_size={max_queue_size} train points")

    async def put_sample(self, sample: Any, item_count: int | None = None) -> bool:
        """
        Put a batch sample into the queue

        Args:
            sample: Sample data
            item_count: Number of train points represented by the sample.

        Returns:
            bool: Whether the sample was successfully put into the queue
        """
        async with self._lock:
            train_points = 0 if sample is None else max(1, int(item_count or 1))
            if train_points > self.max_queue_size:
                logger.warning(
                    "Queue item has %s train points, larger than max_queue_size=%s",
                    train_points,
                    self.max_queue_size,
                )

            # If weighted queue is full, remove oldest samples until the new item fits.
            is_drop = False
            while self.queue and self.queue_train_points + train_points > self.max_queue_size:
                _, dropped_train_points = self.queue.popleft()
                self.queue_train_points = max(0, self.queue_train_points - dropped_train_points)
                self.dropped_samples += 1
                self.dropped_train_points += dropped_train_points
                is_drop = True
                logger.warning("Queue full, dropped sample with %s train points", dropped_train_points)

            self.queue.append((sample, train_points))
            self.queue_train_points += train_points
            self.total_produced += 1
            self.total_produced_train_points += train_points
            ...
            if self.total_produced % 100 == 0:
                print(
                    ...
                    f"queue_entries={len(self.queue)}, "
                    f"queue_train_points={self.queue_train_points}"
                )
            if is_drop:
                return False
            return True
    ...
    async def get_sample(self) -> Any | None:
        ...
        async with self._lock:
            ...
            # Get one sample
            data, train_points = self.queue.popleft()
            self.queue_train_points = max(0, self.queue_train_points - train_points)
            self.total_consumed += 1
            self.total_consumed_train_points += train_points
            return data, self.queue_train_points

    async def get_queue_size(self) -> int:
        """Get current queue size in train points."""
        async with self._lock:
            return self.queue_train_points

    async def get_statistics(self) -> dict[str, Any]:
        """Get queue statistics"""
        async with self._lock:
            return {
                "queue_size": self.queue_train_points,
                "queue_entry_count": len(self.queue),
                "total_produced": self.total_produced,
                "total_consumed": self.total_consumed,
                "total_produced_train_points": self.total_produced_train_points,
                "total_consumed_train_points": self.total_consumed_train_points,
                "dropped_samples": self.dropped_samples,
                "dropped_train_points": self.dropped_train_points,
                "max_queue_size": self.max_queue_size,
                "max_queue_train_points": self.max_queue_size,
            }
    ...
    async def clear_queue(self):
        ...
        async with self._lock:
            ...
            cleared_train_points = self.queue_train_points
            self.queue.clear()
            self.queue_train_points = 0
            logger.info(f"Cleared {cleared_count} samples / {cleared_train_points} train points from queue")
    ...
    async def get_memory_usage(self) -> dict:
        ...
        async with self._lock:
            ...
            if sample_count > 0:
                ...
                sample, _train_points = list(self.queue)[0]
                try:
            ...
            return {
                ...
                "queue_train_points": self.queue_train_points,
...
class MessageQueueClient:
    ...
    async def put_sample(self, sample: Any, item_count: int | None = None) -> bool:
        ...
        future = self.queue_actor.put_sample.remote(sample, item_count)
